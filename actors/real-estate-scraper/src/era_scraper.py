"""
era_scraper.py — ERA.com.tr tüm ilan kategorileri (konut + ticari + devren)

CB/Century21 ile aynı HTML motoru (SHB Portal ağı).
"""
...
def _parse_coordinates(html: str) -> tuple[Optional[float], Optional[float]]:
    m = re.search(
        ...
        r"\s*;\s*.*?googleMapOperations\.lng\s*=\s*'([0-9,.-]+)'",
...
def _parse_description(html: str) -> Optional[str]:
    for pat in [
        r"<h2[^>]*>\s*A&Ccedil;IKLAMA\s*</h2>([\s\S]{50,6000}?)<h3[^>]*>",
        r"<h2[^>]*>\s*AÇIKLAMA\s*</h2>([\s\S]{50,6000}?)<h3[^>]*>",
    ]:
...
def scrape_all(
    ...
    for cat_name, cat_path in CATEGORIES.items():
        if cat_name in collected and collected[cat_name]:
            print(f"[ERA:{cat_name}] {len(collected[cat_name])} URL checkpoint'ten yüklendi")
            continue
        urls: list[str] = []
        try:
            html = fetch_html(_listing_page_url(cat_path, 1))
        except Exception as exc:
            print(f"[ERA:{cat_name}] Liste alınamadı: {exc}")
            collected[cat_name] = []
        ...
        print(f"[ERA:{cat_name}] Toplam sayfa: {total_pages}")
        seen: set[str] = set()
        ...
        if total_pages > 1:
            def _fetch_era_page(page_num: int, _cp=cat_path, _d=delay) -> tuple[int, list[str]]:
            ...
            with ThreadPoolExecutor(max_workers=4) as page_pool:
                page_futs = {page_pool.submit(_fetch_era_page, p): p for p in range(2, total_pages + 1)}
                for fut in as_completed(page_futs):
                    ...
                    try:
                        ...
                        print(f"[ERA:{cat_name}] Sayfa {p_num} hatası: {exc}")
    ...
    print(f"[ERA] Toplam: {len(all_urls)} | Tamamlanan: {len(done_urls)} | Kalan: {len(remaining)}")
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as executor:
        ...
        for future in as_completed(all_futures):
            ...
            try:
                ...
                print(f"  [ERA] [{done_count}/{total}] HATA {url} -> {exc}")
                continue
            ...
            try:
                ...
                break
            done_urls.add(url)
            print(f"  [ERA] [{done_count}/{total}] OK {url}")
            save_checkpoint(checkpoint_path, {"collected": collected, "done_urls": list(done_urls)})
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="ERA.com.tr scraper")
    p.add_argument("--max-pages", type=int, default=0)
    p.add_argument("--delay", type=float, default=1.5)
    p.add_argument("--csv", default="data/era_latest.csv")
    args = p.parse_args()
    result = scrape_all(Path(args.csv), max_pages=args.max_pages, delay=args.delay)
    print(f"\n[ERA] Tamamlandı: {len(result)} ilan")
